...
void create_or_join(RenderWindow &window, Player &player)
{
    Object bg("bg1.png", {0, 0}, {screenResolution.x / 1366.f, screenResolution.y / 768.f});

    Button Create("Create", {screenResolution.x / 2 - 100, screenResolution.y / 2}, back_C, text_C);
    Button Join("Join", {screenResolution.x / 2 + 50, screenResolution.y / 2}, back_C, text_C);
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            if (event.type == Event::MouseButtonPressed && event.mouseButton.button == Mouse::Left)
            {
                ...
                if (Create.isClicked(mousePos))
                {
                    return;
                }

                if (Join.isClicked(mousePos))
                {
                    return;
                }
            }
        ...
        displayText("      Select Option : ", {screenResolution.x / 2 - 120, screenResolution.y / 2 - 50}, window, 30, Color::White);
...
bool ask_bcz_present(RenderWindow &window)
{
    Object bg("bg1.png", {0, 0}, {screenResolution.x / 1366.f, screenResolution.y / 768.f});
    ...
    while (window.isOpen())
    {
        ...
        displayText("Username is not available", {screenResolution.x / 2 - 140, screenResolution.y / 2 - 100}, window, 30, Color::White);
        displayText("    Wanna Try Again", {screenResolution.x / 2 - 120, screenResolution.y / 2 - 50}, window, 30, Color::White);
...
void Game_Menu(RenderWindow &window)
{
    ...
    while (database.isPresent(name))
    {

        if (ask_bcz_present(window))
            name = askName(window);
        else
            return;
    }

    database.appendUserToFile(name);
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            if (event.type == Event::MouseButtonPressed && event.mouseButton.button == Mouse::Left)
            {
                ...
                {
                    b6.ChangeColor();
                    savePlayerData(player, player.name);
                    return;
                }
            }
        }
...
void ContinuedGame(RenderWindow &window)
{
    ...
    cout << "Guild ID: " << ((player.getGuildId() == 0) ? "not Joined any" : to_string(player.getGuildId())) << endl;

    cout << "Guild Name: " << ((player.getGuildName() == "") ? "not Joined any" : player.getGuildName()) << endl;

    Game_Menu(window, player);
}
